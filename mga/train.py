...
Usage:
  ...
  python -m mga.train --model mga --n 4096 --cycles 2 --steps 8000 --bs 64 \
      --lr 1e-3 --exitloss --posxattn --ema 0.999 --bf16 \
      --save runs/ckpt_4096_T2.pt --ckpt_every 1000
  python -m mga.train --model local --n 4096
  python -m mga.train --model full --n 512   # fp32 CPU/MPS also works
"""
import argparse
import contextlib
import json
...
def make_batch(args, g, device):
    if args.task == "passkey":
        return data.passkey_batch(args.bs, args.n, g, device)
    return data.copying_batch(args.bs, args.n, g, device)


def loss_and_acc(model, idx, tgt, mask, exit_w=None):
    """CE on masked positions; with exit_w (mga only), weighted sum of
    per-cycle exit losses. Accuracy is always from the last cycle."""
    ...
    return loss, hit.float().mean().item(), hit.all(1).float().mean().item()
...
def leak_test():
    ...
    not change. Run on CPU for determinism. The single most important test
    in this codebase."""
    ...
    ok_all = True
    for name in ["mga", "full", "local"]:
        args = argparse.Namespace(model=name, n=n, d=64, heads=2, b=16, k=1,
                                  cycles=2, unshared=False, depth=4)
        ...
        ok = err < 1e-4
        ok_all &= ok
        print(f"[leak] {name:4s} max-logit-diff on past positions: {err:.2e} "
...
def overfit_test(device):
    ...
    args = argparse.Namespace(model="mga", n=512, d=128, heads=4, b=16, k=1,
                              cycles=2, unshared=False, depth=4, task="passkey", bs=32)
    torch.manual_seed(0)
    model = build(args, device=device)
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3)
    g = torch.Generator().manual_seed(1)
    idx, tgt, mask = make_batch(args, g, device)
    model.train()
    for step in range(400):
        loss, pd, em = loss_and_acc(model, idx, tgt, mask)
        opt.zero_grad()
        loss.backward()
        opt.step()
...
def train(args, device):
    ...
    for step in range(start_step, args.steps):
        ...
        idx, tgt, mask = make_batch(args, g_train, device)
        model.train()
        with amp_ctx(args, device):
            loss, pd, em = loss_and_acc(model, idx, tgt, mask, exit_w)
        opt.zero_grad()
        loss.backward()
        ...
        if step % 100 == 0:
            rec = dict(step=step, loss=round(loss.item(), 4), digit_acc=round(pd, 4),
                       exact=round(em, 4), sec=round(time.time() - t0, 1))
        ...
        if step % args.eval_every == 0 or step == args.steps - 1:
            ...
            with torch.no_grad(), amp_ctx(args, device):
                for _ in range(4):
                    idx, tgt, mask = make_batch(args, g_eval, device)
                    _, pd, em = loss_and_acc(model, idx, tgt, mask)
                    ems.append(em)
                    pds.append(pd)
...
def main():
    ...
    ap.add_argument("--task", default="passkey", choices=["passkey", "copying"])
    ...
    ap.add_argument("--ema", type=float, default=0.0,
                    help="EMA decay for eval weights (0 = off), e.g. 0.999")
