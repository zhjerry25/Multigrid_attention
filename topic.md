...
1. ~~PyTorch 原型（~300 行）：causal V-cycle block，支持 T cycles、shifted blocking、共享/独立权重切换。~~ 已完成（`mga/`，泄漏测试 0 diff 通过，过拟合 200 步满分）。
2. 合成任务跑 H0（passkey + copying + associative recall）。进行中：n=512 上 full=0.98 / local=0.00 / mga 在学；判别性基线成立。
3. 过了生死线 → PG-19 语言建模 + 外推实验（H2）。
4. 正式实验（远程 GPU）前先完成 v0.1 设计完善，再上卡。
