...
1. **不动点训练（复活 T 旋钮）**：训练时随机化 T（T-dropout），目标 = 解成为吸引不动点——任意 T≥2 收敛而不过冲。检验标准：cycles_exact 在 T>训练值时不衰减。接 DEQ/不动点文献，是把"迭代求解"叙事做实的钥匙。
2. **AMR 选择路径（主线）**：read 除摘要外，对 top-m 摘要块开放原始 token 直连——一举三得：治理 16k+ 的路由对比度衰减、给出检索保真上限、顺带把 read 从 O(n²/b²) 降到线性（65k+ 前置）。
