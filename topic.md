...
**AMR 验尸与否决（2026-09-14，用户判决）**：dense 接手实验（A1）仍远慢于 dense（1500 步 eval exact 0.14 vs dense ~0.98）——ST+探索也救不回。根因：**贪婪树下降让 scorer 的学习被自己的选择绑架（beam myopia），探索被锁死在 beam 内**；三层补丁复杂度爆炸仍治不好。AMR 否决（代码留 `--amr` 审计）。唯一存活的部件：fine fanout（token 级直连），移植进 SparseRead。

## v0.2' SparseRead（当前主线，`--sparse`，2026-09-14）

**设计原则（AMR 验尸长出）**：错的是"选择局部化"，不是"读取稀疏化"。**让选择全局且便宜，只让读取稀疏。**

1. **全局块共享选择**：每个 query 块对**全部**摘要打分选 top-m（代价 (n/b)²，65k 时仅 16.7M 对）。每步全局重打分 = 免费探索（MoBA 已工业验证的同族机制），不存在 beam 锁定。
2. **稀疏读取**：块内 token attend 选中的 m 个摘要（默认 64）→ O(n·m) 线性；m→n/b 时退化为 dense（**精度-线性前沿的刻度尺**，消融白拿）。
3. **fine fanout**：m 中 top-mf（默认 4）展开为原始 token 直连——token 级精度，治 16k 深位衰减。
4. **因果性**（泄漏测试抓出两处，已修）：①块代表 query 用**前一块**的均值（块内均值会让未来 token 的 query 污染过去 token 的选 key）；②fine 选择对无效（掩码）条目做掩码（开头块有效摘要 <mf 时，否则假选中映射到未来块）。

**验收门**（与 AMR 相同）：512 点火趋势 ≈ dense（m≈全量时）→ 4096 resume ≥0.96 → 16k 深位桶提升 → 65k 前向可跑（选择 (n/b)² + 读取 O(n·m)，显存可行）。

**S1/S2 结果（2026-09-14，远程）**：S1 @512 点火成功（eval exact 0.9102@3000，机制无罪 ✓）。S2 @4096 m=64/257：零样本 0.4844 → 1000 步后 eval exact **0.8828** 且终点仍在爬升（train exact 已见 0.9688；resume 只给了 1000 步 + 余弦尾声 lr）。**用户判读：架构没问题，是训练预算/调度小问题，加长余弦的 4096 阶段可到 0.96**——待 S2'（fresh cosine @4096）确认。depth 四桶 [0.882, 0.926, 0.868, 0.862] 无深位塌陷；cycles_exact [0.188, 0.969]（cycle 1 比 dense 分担更多）。注意：sparse 在 512/4096 单步慢于 dense（选择开销主导），其价值区间在 16k+ 与 65k。
