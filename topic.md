...
| LM @4096 kq=4（带宽） | bpc 1.5808 | 带宽不是瓶颈（字符级无那么多内容要运） |
| LM @4096 m=257（稠密 read） | bpc 1.5435 | 稀疏税仅 0.012，可忽略 |

**架构冻结（v0.5, 2026-09-18）**：V-cycle + halo 平滑 + SparseRead(m=64, mf=4) + T=2 + kq=1 + 全共享权重。全部开放旋钮已闭合（带宽无税、稀疏微税、循环必要、边界修复），后续工作 = 主评测 → 消融（见 `map.md`）。
...
**架构已冻结（v0.5）**，进入论文轨道：实验集合、基线矩阵、严谨性纪律、时间线全部见 **`map.md`**（论文路线图与实验集合，2026-09-18 起逐步讨论执行）。远期（论文外）：增量推理调度器、chunked top-k、层级选择（>1M）。
